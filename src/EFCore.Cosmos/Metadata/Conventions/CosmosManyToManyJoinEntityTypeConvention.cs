    ...
    IEntityTypeAnnotationChangedConvention
{
        ...
        IConventionEntityTypeBuilder joinEntityTypeBuilder)
    {
        ...
        var dependentProperties = principalProperties.Select(p => joinEntityTypeBuilder.Property(p!.ClrType, p.Name)!.Metadata).ToList();
        joinEntityTypeBuilder.HasPartitionKey(dependentProperties.Select(p => p.Name).ToList());

        CreateSkipNavigationForeignKey(skipNavigation, joinEntityTypeBuilder, dependentProperties);
        CreateSkipNavigationForeignKey(skipNavigation.Inverse!, joinEntityTypeBuilder, dependentProperties);
    }
        ...
        List<IConventionProperty> partitionKeyProperties)
    {
        ...
        if (partitionKeyProperties.Any(p => skipNavigation.ForeignKey?.Properties.Select(e => e.Name).Contains(p.Name) == true))
        {
            return;
        }
