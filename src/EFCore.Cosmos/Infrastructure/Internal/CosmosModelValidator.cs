...
public class CosmosModelValidator : ModelValidator
{
                ...
                if (partitionKeyStoreNames.Count != storeNames.Count)
                {
                    throw new InvalidOperationException(
                        CosmosStrings.NoPartitionKey(
                            storeNames.Count < partitionKeyStoreNames.Count
                                ? entityType.DisplayName()
                                : firstEntityType.DisplayName(), container));
                }
        ...
        IDiagnosticsLogger<DbLoggerCategory.Model.Validation> logger)
    {
        foreach (var entityType in model.GetEntityTypes())
        {
            ...
            else if (partitionKeyPropertyNames.Count > 3)
            {
                throw new InvalidOperationException(
                    CosmosStrings.PartitionKeyTooManyParts(entityType.DisplayName(), string.Join(',', partitionKeyPropertyNames)));
            }
            else
            {
